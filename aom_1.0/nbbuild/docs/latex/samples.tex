...
289 \}
\end{DoxyCodeInclude}
 \hypertarget{example_svc_encoder_rtc}{}\subsection{svc\+\_\+encoder\+\_\+rtc}\label{example_svc_encoder_rtc}
...
202   printf(\textcolor{stringliteral}{"Total number of processed frames: %d\(\backslash\)n\(\backslash\)n"}, frame\_cnt - 1);
203   printf(\textcolor{stringliteral}{"Rate control layer stats for %d layer(s):\(\backslash\)n\(\backslash\)n"}, ts\_number\_layers);
...
219       printf(\textcolor{stringliteral}{"For layer#: %d %d \(\backslash\)n"}, sl, tl);
220       printf(\textcolor{stringliteral}{"Bitrate (target vs actual): %d %f\(\backslash\)n"}, rc->layer\_target\_bitrate[i],
221              rc->layer\_encoding\_bitrate[i]);
222       printf(\textcolor{stringliteral}{"Average frame size (target vs actual): %f %f\(\backslash\)n"}, rc->layer\_pfb[i],
223              rc->layer\_avg\_frame\_size[i]);
224       printf(\textcolor{stringliteral}{"Average rate\_mismatch: %f\(\backslash\)n"}, rc->layer\_avg\_rate\_mismatch[i]);
225       printf(
226           \textcolor{stringliteral}{"Number of input frames, encoded (non-key) frames, "}
227           \textcolor{stringliteral}{"and perc dropped frames: %d %d %f\(\backslash\)n"},
...
239   printf(\textcolor{stringliteral}{"Short-time stats, for window of %d frames:\(\backslash\)n"}, rc->window\_size);
240   printf(\textcolor{stringliteral}{"Average, rms-variance, and percent-fluct: %f %f %f\(\backslash\)n"},
...
274       \textcolor{keywordflow}{if} (frame\_cnt % 2 == 0) \{
...
289       \textcolor{keywordflow}{if} (frame\_cnt % 4 == 0) \{
...
294       \} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} ((frame\_cnt - 1) % 4 == 0) \{
...
298       \} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} ((frame\_cnt - 2) % 4 == 0) \{
...
303       \} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} ((frame\_cnt - 3) % 4 == 0) \{
...
319       \textcolor{keywordflow}{if} (frame\_cnt % 4 == 0) \{
...
325       \} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} ((frame\_cnt - 1) % 4 == 0) \{
...
329       \} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} ((frame\_cnt - 2) % 4 == 0) \{
...
334       \} \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} ((frame\_cnt - 3) % 4 == 0) \{
...
345       \textcolor{keywordflow}{if} (frame\_cnt % 2 == 0) \{
...
423     die(\textcolor{stringliteral}{"Usage: %s <infile> <outfile> <codec\_type(av1)> <width> <height> "}
...
434   \textcolor{keywordflow}{if} (width < 16 || width % 2 || height < 16 || height % 2) \{
435     die(\textcolor{stringliteral}{"Invalid resolution: %d x %d"}, width, height);
436   \}
437 
438   layering\_mode = (int)strtol(argv[12], NULL, 0);
439   \textcolor{keywordflow}{if} (layering\_mode < 0 || layering\_mode > 13) \{
440     die(\textcolor{stringliteral}{"Invalid layering mode (0..12) %s"}, argv[12]);
441   \}
442 
...
463     printf(\textcolor{stringliteral}{"Failed to get config: %s\(\backslash\)n"}, \hyperlink{group__codec_ga83c6525528574d1a88e73a9f605c9115}{aom\_codec\_err\_to\_string}(res));
...
517     die(\textcolor{stringliteral}{"Invalid value for error resilient (0, 1): %d."}, error\_resilient);
518   \}
...
532       die(\textcolor{stringliteral}{"Incorrect width or height: %d x %d"}, cfg.\hyperlink{structaom__codec__enc__cfg_a80cb459c5ef3c7e1516f617c4c9d6eab}{g\_w}, cfg.\hyperlink{structaom__codec__enc__cfg_a37b0f57b63bec8d133df8901d4407ee6}{g\_h});
533     \}
...
536       die(\textcolor{stringliteral}{"Incorrect framerate: numerator %d denominator %d"},
...
552 
553       snprintf(file\_name, \textcolor{keyword}{sizeof}(file\_name), \textcolor{stringliteral}{"%s\_%d.av1"}, argv[2], i);
554       outfile[i] = aom\_video\_writer\_open(file\_name, kContainerIVF, &info);
555       \textcolor{keywordflow}{if} (!outfile[i]) die(\textcolor{stringliteral}{"Failed to open %s for writing"}, file\_name);
...
664               if (frame\_cnt % rc.window\_size == 0) \{
665                 rc.window\_count += 1;
666                 rc.avg\_st\_encoding\_bitrate += sum\_bitrate / rc.window\_size;
667                 rc.variance\_st\_encoding\_bitrate +=
668                     (sum\_bitrate / rc.window\_size) *
669                     (sum\_bitrate / rc.window\_size);
670                 sum\_bitrate = 0.0;
671               \}
672             \}
673             \textcolor{comment}{// Second shifted window.}
674             \textcolor{keywordflow}{if} (frame\_cnt > rc.window\_size + rc.window\_size / 2 &&
675                 slx == ss\_number\_layers - 1) \{
676               sum\_bitrate2 += 0.001 * 8.0 * pkt->\hyperlink{structaom__codec__cx__pkt_afb379cd4bfa7692d1d6e85f4e4b2b410}{data}.\hyperlink{structaom__codec__cx__pkt_a4180a6ae59b0d295bc915d4689df4cb0}{frame}.sz * framerate;
677               \textcolor{keywordflow}{if} (frame\_cnt > 2 * rc.window\_size &&
678                   frame\_cnt % rc.window\_size == 0) \{
679                 rc.window\_count += 1;
680                 rc.avg\_st\_encoding\_bitrate += sum\_bitrate2 / rc.window\_size;
681                 rc.variance\_st\_encoding\_bitrate +=
682                     (sum\_bitrate2 / rc.window\_size) *
683                     (sum\_bitrate2 / rc.window\_size);
684                 sum\_bitrate2 = 0.0;
685               \}
686             \}
687             \textcolor{keywordflow}{break};
688           \textcolor{keywordflow}{default}: \textcolor{keywordflow}{break};
689         \}
690       \}
691     \}  \textcolor{comment}{// loop over spatial layers}
692     ++frame\_cnt;
693     pts += frame\_duration;
694   \}
695   close\_input\_file(&input\_ctx);
696   printout\_rate\_control\_summary(&rc, frame\_cnt, ss\_number\_layers,
697                                 ts\_number\_layers);
698   printf(\textcolor{stringliteral}{"\(\backslash\)n"});
699   printf(\textcolor{stringliteral}{"Frame cnt and encoding time/FPS stats for encoding: %d %f %f\(\backslash\)n"},
700          frame\_cnt, 1000 * (\textcolor{keywordtype}{float})cx\_time / (\textcolor{keywordtype}{double})(frame\_cnt * 1000000),
701          1000000 * (\textcolor{keywordtype}{double})frame\_cnt / (\textcolor{keywordtype}{double})cx\_time);
702 
703   \textcolor{keywordflow}{if} (\hyperlink{group__codec_ga9b60e186f61ba2d6ab2b8069b76a15c5}{aom\_codec\_destroy}(&codec)) die\_codec(&codec, \textcolor{stringliteral}{"Failed to destroy codec"});
704 
705   \textcolor{comment}{// Try to rewrite the output file headers with the actual frame count.}
706   \textcolor{keywordflow}{for} (i = 0; i < ss\_number\_layers * ts\_number\_layers; ++i)
707     aom\_video\_writer\_close(outfile[i]);
708 
709   \textcolor{keywordflow}{if} (input\_ctx.file\_type != FILE\_TYPE\_Y4M) \{
710     \hyperlink{aom__image_8h_afff22f7f3eb9409c5b678d1962f110a8}{aom\_img\_free}(&raw);
711   \}
712   \textcolor{keywordflow}{return} EXIT\_SUCCESS;
713 \}
\end{DoxyCodeInclude}
...
492   flags |= (cfg.\hyperlink{structaom__codec__enc__cfg_a76a27f44cded1516803a776a0a7e9038}{g\_bit\_depth} > \hyperlink{group__codec_gga6ed0e98eba4651c1ad845e39498e4153a4c7f55539160206a3fbb2a6cfc9ef89c}{AOM\_BITS\_8} || FORCE\_HIGHBITDEPTH\_DECODING)
...
511 
512   \textcolor{keywordflow}{if} (FORCE\_HIGHBITDEPTH\_DECODING) \hyperlink{aom__image_8h_afff22f7f3eb9409c5b678d1962f110a8}{aom\_img\_free}(&raw\_shift);
...
140         (*tile\_idx % output\_frame\_width\_in\_tiles) * tile\_width;
141 
...
178   \textcolor{keywordflow}{if} (!reader) die(\textcolor{stringliteral}{"Failed to open %s for reading."}, argv[1]);
179 
180   \textcolor{keywordflow}{if} (!(outfile = fopen(argv[2], \textcolor{stringliteral}{"wb"})))
181     die(\textcolor{stringliteral}{"Failed to open %s for writing."}, argv[2]);
182 
...
196   printf(\textcolor{stringliteral}{"Using %s\(\backslash\)n"}, \hyperlink{group__codec_ga013b1f6d96b2cf9489396311a7e5179b}{aom\_codec\_iface\_name}(decoder->codec\_interface()));
197 
...
241       snprintf(name, \textcolor{keyword}{sizeof}(name), \textcolor{stringliteral}{"ref\_%d.yuv"}, i);
242       printf(\textcolor{stringliteral}{"writing ref image to %s, %d, %d\(\backslash\)n"}, name, img->\hyperlink{structaom__image_a89f80b1f58d608b9d2080635f4359034}{d\_w}, img->\hyperlink{structaom__image_ab986419a1f0fff93a2dc505f47194988}{d\_h});
...
274   \}
275   printf(\textcolor{stringliteral}{"Read %d frames.\(\backslash\)n"}, num\_frames);
276 
...
316     sscanf(line, \textcolor{stringliteral}{"%d %d %d %d"}, &image\_idx, &ref\_idx, &tc, &tr);
317     \textcolor{keywordflow}{if} (image\_idx >= num\_frames) \{
318       die(\textcolor{stringliteral}{"Tile list image\_idx out of bounds: %d >= %d."}, image\_idx,
319           num\_frames);
320     \}
321     \textcolor{keywordflow}{if} (ref\_idx >= num\_references) \{
322       die(\textcolor{stringliteral}{"Tile list ref\_idx out of bounds: %d >= %d."}, ref\_idx,
323           num\_references);
324     \}
...
329     decode\_tile(&codec, frame, frame\_size, tr, tc, ref\_idx, reference\_images,
330                 &output, &tile\_idx, &output\_bit\_depth, &img, output\_format);
331     \textcolor{keywordflow}{if} (output\_format == YUV1D) \{
332       out = img;
333       \textcolor{keywordflow}{if} (output\_bit\_depth != 0)
334         aom\_shift\_img(output\_bit\_depth, &out, &output\_shifted);
335       aom\_img\_write(out, outfile);
336     \}
337   \}
338 
339   \textcolor{keywordflow}{if} (output\_format != YUV1D) \{
340     \textcolor{comment}{// Write out the last tile list.}
341     \textcolor{keywordflow}{if} (tile\_list\_writes < tile\_list\_cnt) \{
342       out = &output;
343       \textcolor{keywordflow}{if} (output\_bit\_depth != 0)
344         aom\_shift\_img(output\_bit\_depth, &out, &output\_shifted);
345       img\_write\_to\_file(out, outfile, output\_format);
346     \}
347   \}
348 
349   \textcolor{keywordflow}{if} (output\_shifted) \hyperlink{aom__image_8h_afff22f7f3eb9409c5b678d1962f110a8}{aom\_img\_free}(output\_shifted);
350   \textcolor{keywordflow}{if} (output\_format != YUV1D) \hyperlink{aom__image_8h_afff22f7f3eb9409c5b678d1962f110a8}{aom\_img\_free}(&output);
351   \textcolor{keywordflow}{for} (i = 0; i < num\_references; i++) \hyperlink{aom__image_8h_afff22f7f3eb9409c5b678d1962f110a8}{aom\_img\_free}(&reference\_images[i]);
352   \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} f = 0; f < num\_frames; ++f) \{
353     free(frames[f]);
354   \}
355   free(frame\_sizes);
356   free(frames);
357   \textcolor{keywordflow}{if} (\hyperlink{group__codec_ga9b60e186f61ba2d6ab2b8069b76a15c5}{aom\_codec\_destroy}(&codec)) die\_codec(&codec, \textcolor{stringliteral}{"Failed to destroy codec"});
358   aom\_video\_reader\_close(reader);
359   fclose(outfile);
360 
361   \textcolor{keywordflow}{return} EXIT\_SUCCESS;
362 \}
\end{DoxyCodeInclude}
...
903                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a095d0817d339940aa2578924a840fc84}{AV1E\_SET\_ROW\_MT},
904                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5acf4ab1ff2fa8d76a78881ad7f1a1294d}{AV1E\_SET\_TILE\_COLUMNS},
905                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a57f2f2a54f593b398a5e97db7982f817}{AV1E\_SET\_TILE\_ROWS},
906                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a174ffbc2e91a020a8225900e38894137}{AV1E\_SET\_ENABLE\_TPL\_MODEL},
907                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a327fb270f37bbb008b0f146542ce7483}{AOME\_SET\_ARNR\_MAXFRAMES},
908                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5affd2c213f24a12d13bb51e31be993e60}{AOME\_SET\_ARNR\_STRENGTH},
909                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5aba539b6646278e7ff40f67f38639d294}{AOME\_SET\_TUNING},
910                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5af67f265bf63bf8f1268b3a14ae26606c}{AOME\_SET\_CQ\_LEVEL},
911                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a9e1965ffa40d2d87556b65748c63adcc}{AOME\_SET\_MAX\_INTRA\_BITRATE\_PCT},
912                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a54b7c950e51f39a3cb7344a2665b9929}{AV1E\_SET\_MAX\_INTER\_BITRATE\_PCT},
913                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5ade327180f01d27d22abd94c1f0a8bc9e}{AV1E\_SET\_GF\_CBR\_BOOST\_PCT},
914                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5afe875c6bb02f236be503c8c7b1f15875}{AV1E\_SET\_LOSSLESS},
915                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a6879fe104e23de00f034ed11eb605031}{AV1E\_SET\_ENABLE\_CDEF},
916                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a2ea70b4b379e701a4b3b7cc33eb05e65}{AV1E\_SET\_ENABLE\_RESTORATION},
917                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5af3db44e9871c69defc5ae2cc6c5e3ba0}{AV1E\_SET\_ENABLE\_RECT\_PARTITIONS},
...
948                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a374b5581d0b68d5f8cc3e3f73495cfbb}{AV1E\_SET\_DISABLE\_TRELLIS\_QUANT},
949                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5ae63993934cdc5aa36efeec4df87a577e}{AV1E\_SET\_ENABLE\_QM},
950                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5ae611944ba148b6446d4387dd9b452917}{AV1E\_SET\_QM\_MIN},
951                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5ad897674e1b68b24ec14a01a4e3edacc7}{AV1E\_SET\_QM\_MAX},
952                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5abc82eae151e4a5050dbc324b5d16be2d}{AV1E\_SET\_REDUCED\_TX\_TYPE\_SET},
953                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a56dd8bdd23f09e24e2afea7633f20e74}{AV1E\_SET\_INTRA\_DCT\_ONLY},
954                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5ac3dcd39257d82c1be99a8752bef6f20b}{AV1E\_SET\_INTER\_DCT\_ONLY},
955                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a335eed63dd36dd6e9a83a462c3c15fe1}{AV1E\_SET\_INTRA\_DEFAULT\_TX\_ONLY},
956                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a953968db8a041da6fe727add815ad53e}{AV1E\_SET\_QUANT\_B\_ADAPT},
957                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a542a607e7d8c6a3d8244136b3e09da4b}{AV1E\_SET\_COEFF\_COST\_UPD\_FREQ},
958                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5afb9a186506424be1020cdd8c79342b7a}{AV1E\_SET\_MODE\_COST\_UPD\_FREQ},
959 \textcolor{preprocessor}{#if CONFIG\_DIST\_8X8}
960                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a7ef16c780349c5fee8281f1e5b4a6718}{AV1E\_SET\_ENABLE\_DIST\_8X8},
961 \textcolor{preprocessor}{#endif}
962                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a465382b6bbca24467739c3c1b94e6483}{AV1E\_SET\_FRAME\_PARALLEL\_DECODING},
963                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5aa0d049453fced5f8079861b16e356c69}{AV1E\_SET\_ERROR\_RESILIENT\_MODE},
964                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a17bd297efc31082157785d68fc5a6067}{AV1E\_SET\_AQ\_MODE},
965                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5ad26eda6211bf3d5fe4f5999ec52b264b}{AV1E\_SET\_DELTAQ\_MODE},
966                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5ab2d0e42bcf19974859e61d8709c5ae09}{AV1E\_SET\_DELTALF\_MODE},
967                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a737368591a37417b108c0706f7510885}{AV1E\_SET\_FRAME\_PERIODIC\_BOOST},
968                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a8d415cd6976ea5860206e1d3cd3dd222}{AV1E\_SET\_NOISE\_SENSITIVITY},
969                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a2625f566fd5c2675eb2bcd40af858652}{AV1E\_SET\_TUNE\_CONTENT},
970                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a277526a4a9b8c413e870ee17ff986e87}{AV1E\_SET\_CDF\_UPDATE\_MODE},
971                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5af64e990728fe37287bac0874d77b22db}{AV1E\_SET\_COLOR\_PRIMARIES},
972                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a336fc3607a5987f9077c634f07b3cd53}{AV1E\_SET\_TRANSFER\_CHARACTERISTICS},
973                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a0111ed659d2cd11827ac5cd1f3d774b0}{AV1E\_SET\_MATRIX\_COEFFICIENTS},
974                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5abc5bb5cf57ba1a1193a0593225e881ff}{AV1E\_SET\_CHROMA\_SAMPLE\_POSITION},
975                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a31bcfcb7057884ceb831146c90086ddd}{AV1E\_SET\_MIN\_GF\_INTERVAL},
976                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a0355cea227ab76f9bd77028a39bbfca8}{AV1E\_SET\_MAX\_GF\_INTERVAL},
977                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a685f831a5a5d08f9b5a14209d55ac129}{AV1E\_SET\_GF\_MAX\_PYRAMID\_HEIGHT},
978                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a8854a06c62269e866d6bac0f3a2e3dc2}{AV1E\_SET\_SUPERBLOCK\_SIZE},
979                                         \hyperlink{group__aom__encoder_ggae78dde67a6d78f332e9bdba0dde42db5a9ac0044ad63e6848a482db04f0858c44}{AV1E\_SET\_NUM\_TG},
...
1000   fprintf(fout, \textcolor{stringliteral}{"Usage: %s <options> -o dst\_filename src\_filename \(\backslash\)n"},
1001           exec\_name);
1002 
...
1032     fprintf(fout, \textcolor{stringliteral}{"    %-6s - %s %s\(\backslash\)n"}, encoder->name,
...
1099   \}
1100 
1101   \textcolor{keywordflow}{if} (rat->\hyperlink{structaom__rational_a7b48174411798c780a15f132c4650839}{num} < 0) die(\textcolor{stringliteral}{"Error: %s must be positive\(\backslash\)n"}, msg);
1102 
1103   \textcolor{keywordflow}{if} (!rat->\hyperlink{structaom__rational_adeddf2ea01c12b7be66536e0a0fb92c5}{den}) die(\textcolor{stringliteral}{"Error: %s has zero denominator\(\backslash\)n"}, msg);
1104 \}
1105 
...
1154         die(\textcolor{stringliteral}{"Error: Unrecognized argument (%s) to --codec\(\backslash\)n"}, arg.val);
...
1159         die(\textcolor{stringliteral}{"Error: Invalid number of passes (%d)\(\backslash\)n"}, global->passes);
...
1164         die(\textcolor{stringliteral}{"Error: Invalid pass selected (%d)\(\backslash\)n"}, global->pass);
...
1216       warn(\textcolor{stringliteral}{"Assuming --pass=%d implies --passes=%d\(\backslash\)n"}, global->pass,
1217            global->pass);
...
1313     \textcolor{keywordflow}{if} (res) fatal(\textcolor{stringliteral}{"Failed to get config: %s\(\backslash\)n"}, \hyperlink{group__codec_ga83c6525528574d1a88e73a9f605c9115}{aom\_codec\_err\_to\_string}(res));
1314 
...
1483         warn(\textcolor{stringliteral}{"non-zero %s option ignored in realtime CBR mode.\(\backslash\)n"}, arg.name);
...
1532         warn(\textcolor{stringliteral}{"option %s ignored in one-pass mode.\(\backslash\)n"}, arg.name);
...
1537         warn(\textcolor{stringliteral}{"option %s ignored in one-pass mode.\(\backslash\)n"}, arg.name);
...
1542         warn(\textcolor{stringliteral}{"option %s ignored in one-pass mode.\(\backslash\)n"}, arg.name);
...
1570         warn(\textcolor{stringliteral}{"non-zero %s option ignored in realtime mode.\(\backslash\)n"}, arg.name);
1571       \}
...
1600     fatal(
1601         \textcolor{stringliteral}{"Stream %d: Specify stream dimensions with --width (-w) "}
...
1614       fatal(\textcolor{stringliteral}{"Stream %d: Output file is required (specify with -o)"},
1615             streami->index);
1616 
...
1622         fatal(\textcolor{stringliteral}{"Stream %d: duplicate output file (from stream %d)"},
...
1631         fatal(\textcolor{stringliteral}{"Stream %d: duplicate stats file (from stream %d)"},
...
1676 \textcolor{preprocessor}{  fprintf(stderr, "    %-28s = %d\(\backslash\)n", #field, stream->config.cfg.field)}
1677 
1678   \textcolor{keywordflow}{if} (stream->index == 0) \{
1679     fprintf(stderr, \textcolor{stringliteral}{"Codec: %s\(\backslash\)n"},
1680             \hyperlink{group__codec_ga013b1f6d96b2cf9489396311a7e5179b}{aom\_codec\_iface\_name}(global->codec->codec\_interface()));
1681     fprintf(stderr, \textcolor{stringliteral}{"Source file: %s File Type: %s Format: %s\(\backslash\)n"},
...
1686     fprintf(stderr, \textcolor{stringliteral}{"\(\backslash\)nStream Index: %d\(\backslash\)n"}, stream->index);
1687   fprintf(stderr, \textcolor{stringliteral}{"Destination file: %s\(\backslash\)n"}, stream->config.out\_fn);
1688   fprintf(stderr, \textcolor{stringliteral}{"Coding path: %s\(\backslash\)n"},
...
1831       fprintf(stderr, \textcolor{stringliteral}{"Error: Tried to set control %d = %d\(\backslash\)n"}, ctrl, value);
1832 
...
1863 \}
1864 
1865 \textcolor{keyword}{static} \textcolor{keywordtype}{void} encode\_frame(\textcolor{keyword}{struct} stream\_state *stream,
1866                          \textcolor{keyword}{struct} AvxEncoderConfig *global, \textcolor{keyword}{struct} \hyperlink{structaom__image}{aom\_image} *img,
1867                          \textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int} frames\_in) \{
1868   \hyperlink{group__encoder_ga958524226c9a65251c9e4f7bb78fc606}{aom\_codec\_pts\_t} frame\_start, next\_frame\_start;
1869   \textcolor{keyword}{struct }\hyperlink{structaom__codec__enc__cfg}{aom\_codec\_enc\_cfg} *cfg = &stream->config.cfg;
1870   \textcolor{keyword}{struct }aom\_usec\_timer timer;
1871 
1872   frame\_start =
1873       (cfg->\hyperlink{structaom__codec__enc__cfg_a10664f1fc5b6ec29b77ee13efeeecdf7}{g\_timebase}.\hyperlink{structaom__rational_adeddf2ea01c12b7be66536e0a0fb92c5}{den} * (int64\_t)(frames\_in - 1) * global->framerate.den) /
1874       cfg->\hyperlink{structaom__codec__enc__cfg_a10664f1fc5b6ec29b77ee13efeeecdf7}{g\_timebase}.\hyperlink{structaom__rational_a7b48174411798c780a15f132c4650839}{num} / global->framerate.num;
1875   next\_frame\_start =
1876       (cfg->\hyperlink{structaom__codec__enc__cfg_a10664f1fc5b6ec29b77ee13efeeecdf7}{g\_timebase}.\hyperlink{structaom__rational_adeddf2ea01c12b7be66536e0a0fb92c5}{den} * (int64\_t)(frames\_in)*global->framerate.den) /
1877       cfg->\hyperlink{structaom__codec__enc__cfg_a10664f1fc5b6ec29b77ee13efeeecdf7}{g\_timebase}.\hyperlink{structaom__rational_a7b48174411798c780a15f132c4650839}{num} / global->framerate.num;
1878 
1879   \textcolor{comment}{/* Scale if necessary */}
1880   \textcolor{keywordflow}{if} (img) \{
1881     \textcolor{keywordflow}{if} ((img->\hyperlink{structaom__image_a6c64b1ab918d80d52eb8f5d6d957e825}{fmt} & \hyperlink{aom__image_8h_a607b37d91f75442f54223ecd85f1b6cb}{AOM\_IMG\_FMT\_HIGHBITDEPTH}) &&
1882         (img->\hyperlink{structaom__image_a89f80b1f58d608b9d2080635f4359034}{d\_w} != cfg->\hyperlink{structaom__codec__enc__cfg_a80cb459c5ef3c7e1516f617c4c9d6eab}{g\_w} || img->\hyperlink{structaom__image_ab986419a1f0fff93a2dc505f47194988}{d\_h} != cfg->\hyperlink{structaom__codec__enc__cfg_a37b0f57b63bec8d133df8901d4407ee6}{g\_h})) \{
1883       \textcolor{keywordflow}{if} (img->\hyperlink{structaom__image_a6c64b1ab918d80d52eb8f5d6d957e825}{fmt} != \hyperlink{aom__image_8h_a930317c04b4bd0a660bb5e744055523ca20a9cf30277260685642b4cfb4e9273b}{AOM\_IMG\_FMT\_I42016}) \{
1884         fprintf(stderr, \textcolor{stringliteral}{"%s can only scale 4:2:0 inputs\(\backslash\)n"}, exec\_name);
...
1907                         \textcolor{stringliteral}{"Stream %d: Failed to encode frame.\(\backslash\)n"}
...
1914     \}
1915   \}
1916   \textcolor{keywordflow}{if} (img && (img->\hyperlink{structaom__image_a89f80b1f58d608b9d2080635f4359034}{d\_w} != cfg->\hyperlink{structaom__codec__enc__cfg_a80cb459c5ef3c7e1516f617c4c9d6eab}{g\_w} || img->\hyperlink{structaom__image_ab986419a1f0fff93a2dc505f47194988}{d\_h} != cfg->\hyperlink{structaom__codec__enc__cfg_a37b0f57b63bec8d133df8901d4407ee6}{g\_h})) \{
1917     \textcolor{keywordflow}{if} (img->\hyperlink{structaom__image_a6c64b1ab918d80d52eb8f5d6d957e825}{fmt} != \hyperlink{aom__image_8h_a930317c04b4bd0a660bb5e744055523cabd778a3d697463e89d12a1117f417b60}{AOM\_IMG\_FMT\_I420} && img->\hyperlink{structaom__image_a6c64b1ab918d80d52eb8f5d6d957e825}{fmt} != 
      \hyperlink{aom__image_8h_a930317c04b4bd0a660bb5e744055523cad28244100a2754409f285b77a3db90a0}{AOM\_IMG\_FMT\_YV12}) \{
1918       fprintf(stderr, \textcolor{stringliteral}{"%s can only scale 4:2:0 8bpp inputs\(\backslash\)n"}, exec\_name);
1919       exit(EXIT\_FAILURE);
1920     \}
1921 \textcolor{preprocessor}{#if CONFIG\_LIBYUV}
1922     \textcolor{keywordflow}{if} (!stream->img)
1923       stream->img =
1924           \hyperlink{aom__image_8h_a570db29fbd122951235a08fe9375f6bb}{aom\_img\_alloc}(NULL, \hyperlink{aom__image_8h_a930317c04b4bd0a660bb5e744055523cabd778a3d697463e89d12a1117f417b60}{AOM\_IMG\_FMT\_I420}, cfg->
      \hyperlink{structaom__codec__enc__cfg_a80cb459c5ef3c7e1516f617c4c9d6eab}{g\_w}, cfg->\hyperlink{structaom__codec__enc__cfg_a37b0f57b63bec8d133df8901d4407ee6}{g\_h}, 16);
1925     I420Scale(
1926         img->\hyperlink{structaom__image_ac54dbc5237ca2914f9ec30105dfbe302}{planes}[\hyperlink{aom__image_8h_a2ad22076006b15cf809edcaa33f6d339}{AOM\_PLANE\_Y}], img->\hyperlink{structaom__image_a6dc693d7dbc9eb06c0cdde307ca58372}{stride}[\hyperlink{aom__image_8h_a2ad22076006b15cf809edcaa33f6d339}{AOM\_PLANE\_Y}],
1927         img->\hyperlink{structaom__image_ac54dbc5237ca2914f9ec30105dfbe302}{planes}[AOM\_PLANE\_U], img->\hyperlink{structaom__image_a6dc693d7dbc9eb06c0cdde307ca58372}{stride}[AOM\_PLANE\_U],
1928         img->\hyperlink{structaom__image_ac54dbc5237ca2914f9ec30105dfbe302}{planes}[AOM\_PLANE\_V], img->\hyperlink{structaom__image_a6dc693d7dbc9eb06c0cdde307ca58372}{stride}[AOM\_PLANE\_V], img->\hyperlink{structaom__image_a89f80b1f58d608b9d2080635f4359034}{d\_w}, img->
      \hyperlink{structaom__image_ab986419a1f0fff93a2dc505f47194988}{d\_h},
1929         stream->img->planes[\hyperlink{aom__image_8h_a2ad22076006b15cf809edcaa33f6d339}{AOM\_PLANE\_Y}], stream->img->stride[
      \hyperlink{aom__image_8h_a2ad22076006b15cf809edcaa33f6d339}{AOM\_PLANE\_Y}],
...
1937                       \textcolor{stringliteral}{"Stream %d: Failed to encode frame.\(\backslash\)n"}
...
1949   ctx\_exit\_on\_error(&stream->encoder, \textcolor{stringliteral}{"Stream %d: Failed to encode frame"},
1950                     stream->index);
1951 \}
1952 
...
1978           fprintf(stderr, \textcolor{stringliteral}{" %6luF"}, (\textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{long})pkt->\hyperlink{structaom__codec__cx__pkt_afb379cd4bfa7692d1d6e85f4e4b2b410}{data}.\hyperlink{structaom__codec__cx__pkt_a4180a6ae59b0d295bc915d4689df4cb0}{frame}.sz);
1979 
...
2018                                   \textcolor{stringliteral}{"Failed to decode frame %d in stream %d"},
...
2040               fprintf(stderr, \textcolor{stringliteral}{"%.3f "}, pkt->\hyperlink{structaom__codec__cx__pkt_afb379cd4bfa7692d1d6e85f4e4b2b410}{data}.\hyperlink{structaom__codec__cx__pkt_a3a38e942aa0da463260a05eb05f51762}{psnr}.psnr[i]);
...
2057 
2058   fprintf(stderr, \textcolor{stringliteral}{"Stream %d PSNR (Overall/Avg/Y/U/V)"}, stream->index);
...
2061   fprintf(stderr, \textcolor{stringliteral}{" %.3f"}, ovpsnr);
2062 
2063   \textcolor{keywordflow}{for} (i = 0; i < 4; i++) \{
2064     fprintf(stderr, \textcolor{stringliteral}{" %.3f"}, stream->psnr\_totals[i] / stream->psnr\_count);
2065   \}
2066   \textcolor{keywordflow}{if} (bps > 0) \{
2067     fprintf(stderr, \textcolor{stringliteral}{" %7"} PRId64 \textcolor{stringliteral}{" bps"}, bps);
2068   \}
2069   fprintf(stderr, \textcolor{stringliteral}{" %7"} PRId64 \textcolor{stringliteral}{" ms"}, stream->cx\_time / 1000);
...
2117                           \textcolor{stringliteral}{"Stream %d: Encode/decode mismatch on frame %d at"}
2118                           \textcolor{stringliteral}{" Y[%d, %d] \{%d/%d\},"}
2119                           \textcolor{stringliteral}{" U[%d, %d] \{%d/%d\},"}
2120                           \textcolor{stringliteral}{" V[%d, %d] \{%d/%d\}"},
...
2140     secs = etl;
2141 
2142     fprintf(stderr, \textcolor{stringliteral}{"[%3s %2"} PRId64 \textcolor{stringliteral}{":%02"} PRId64 \textcolor{stringliteral}{":%02"} PRId64 \textcolor{stringliteral}{"] "}, label,
2143             hours, mins, secs);
2144   \} \textcolor{keywordflow}{else} \{
2145     fprintf(stderr, \textcolor{stringliteral}{"[%3s  unknown] "}, label);
2146   \}
2147 \}
2148 
...
2213       die(\textcolor{stringliteral}{"Error: Unrecognized option %s\(\backslash\)n"}, *argi);
2214 
...
2347           fprintf(stderr,
2348                   \textcolor{stringliteral}{"Warning: automatically updating bit depth to %d to "}
...
2367         fprintf(stderr,
2368                 \textcolor{stringliteral}{"Warning: automatically updating to profile %d to "}
...
2387           die(\textcolor{stringliteral}{"Stream %d: Must specify --fpf when --pass=%d"}
...
2471           fprintf(stderr, \textcolor{stringliteral}{"\(\backslash\)rPass %d/%d "}, pass + 1, global.passes);
2472 
2473           \textcolor{keywordflow}{if} (stream\_cnt == 1)
2474             fprintf(stderr, \textcolor{stringliteral}{"frame %4d/%-4d %7"} PRId64 \textcolor{stringliteral}{"B "}, frames\_in,
2475                     streams->frames\_out, (int64\_t)streams->nbytes);
2476           \textcolor{keywordflow}{else}
2477             fprintf(stderr, \textcolor{stringliteral}{"frame %4d "}, frames\_in);
2478 
2479           fprintf(stderr, \textcolor{stringliteral}{"%7"} PRId64 \textcolor{stringliteral}{" %s %.2f %s "},
...
2578         fprintf(stderr,
2579                 \textcolor{stringliteral}{"\(\backslash\)rPass %d/%d frame %4d/%-4d %7"} PRId64 \textcolor{stringliteral}{"B %7"} PRId64
2580                 \textcolor{stringliteral}{"b/f %7"} PRId64
2581                 \textcolor{stringliteral}{"b/s"}
2582                 \textcolor{stringliteral}{" %7"} PRId64 \textcolor{stringliteral}{" %s (%.2f fps)\(\backslash\)033[K\(\backslash\)n"},
...
2651       \textcolor{keywordflow}{if} (stream->mismatch\_seen) \{
2652         fprintf(f, \textcolor{stringliteral}{"First mismatch occurred in frame %d\(\backslash\)n"},
2653                 stream->mismatch\_seen);
2654       \} \textcolor{keywordflow}{else} \{
2655         fprintf(f, \textcolor{stringliteral}{"No mismatch detected in recon buffers\(\backslash\)n"});
2656       \}
2657       fclose(f);
2658     \}
2659   \}
2660 \textcolor{preprocessor}{#endif}
2661 
2662   \textcolor{keywordflow}{if} (allocated\_raw\_shift) \hyperlink{aom__image_8h_afff22f7f3eb9409c5b678d1962f110a8}{aom\_img\_free}(&raw\_shift);
2663   \hyperlink{aom__image_8h_afff22f7f3eb9409c5b678d1962f110a8}{aom\_img\_free}(&raw);
2664   free(argv);
2665   free(streams);
2666   \textcolor{keywordflow}{return} res ? EXIT\_FAILURE : EXIT\_SUCCESS;
2667 \}
\end{DoxyCodeInclude}
