...
file = open("../aom/run_av1_gp_noSIMD.sh","w")
for qp in [19,24,29,34]:
	for yuv in yuvs:
		...
		#qp = map(float,qp)
		linha = "%s/pesquisa_ucpel/aom_1.0_noSIMD/build/aomenc -p 1 -t 1 --psnr -v --fps=%s/1 -w %s -h %s --min-q=%s --max-q=%s --limit=%s --good --tune=psnr --lag-in-frames=0 --end-usage=q --min-gf-interval=16 --max-gf-interval=16 -b 8 -o %s/testesAV1/gpbin/%s_noSIMD.bin %s/%s 2> %s/testesAV1/gpout/%s_qp%s_av1_noSIMD_out && gprof %s/pesquisa_ucpel/aom_1.0/build/aomenc %s/pesquisa_ucpel/codes/run-sh/aom/gmon.out > %s/testesAV1/gprof/gprof_%s_qp%s_av1_noSIMD.txt"%(homepath,fr,w,h,qp,qp+8,f,homepath,nome,yuvpath,yuv,homepath,yuv,qp+3,homepath,homepath,homepath,yuv,qp+3)
		print >> file, linha
		file.close