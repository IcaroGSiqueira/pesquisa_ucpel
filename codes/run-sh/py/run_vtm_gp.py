...
for yuv in yuvs:
	for qp in [22,27,32,37]:
		...
		linha = "%s/pesquisa_ucpel/VTM_5.0/bin/EncoderAppStatic_gp -c %s/pesquisa_ucpel/VTM_5.0/cfg/encoder_randomaccess_vtm.cfg --InputFile=%s/%s --SourceHeight=%s --SourceWidth=%s -f %s -fr %s -q %s --BitstreamFile=%s/testesVVC/bingp/%s.bin  > %s/testesVVC/outgp/%s_qp%s_vtm_out && gprof %s/pesquisa_ucpel/VTM_5.0/bin/EncoderAppStatic_gp %s/pesquisa_ucpel/codes/run-sh/gmon.out > %s/testesVVC/gprof/gprof_%s_qp%s_vtm.txt" %(homepath,homepath,yuvpath,yuv,h,w,f,fr,qp,homepath,nome,homepath,yuv,qp,homepath,homepath,homepath,yuv,qp)
		print >> file, linha
		file.close